...
impl Renderer {
	...
	fn with_config(config: RendererBuilder) -> Renderer {
				...
				iter![
					gfx_hal::pso::DescriptorSetLayoutBinding {
						...
						ty: gfx_hal::pso::DescriptorType::Image {
							ty: gfx_hal::pso::ImageDescriptorType::Sampled { with_sampler: false },
						},
						count: 1,
		...
		let allocator = GpuAllocator::new(config_fn(&alloc_props), alloc_props);
...
impl RendererBuilder {
	...
	pub fn allocation_config(
		mut self,
		alloc_config: impl FnOnce(&gpu_alloc::DeviceProperties) -> gpu_alloc::Config + 'static,
	) -> RendererBuilder {
...
impl WindowTarget {
	...
	pub fn pixel(&self, x: i32, y: i32) -> Vector2 {
		Vector2::new(
			(x * 2) as f32 / self.swapchain_config.extent.width as f32 - 1.0,
			-((y * 2) as f32 / self.swapchain_config.extent.height as f32 - 1.0),
		)
	}
...
struct WindowFrame<'a> {
	...
	swap_chain_frame:
		ManuallyDrop<<backend::Surface as gfx_hal::window::PresentationSurface<backend::Backend>>::SwapchainImage>,
}
...
impl Texture {
	...
	fn _from_rgba(context: Rc<Renderer>, data: &[u8], (width, height): (u32, u32)) -> Texture {
		...
		let sampler = unsafe {
			context.device.create_sampler(&gfx_hal::image::SamplerDesc::new(
		...
		unsafe {
			...
			context.queue_groups.borrow_mut()[0].queues[0].submit(
				iter![&command_buffer],
				iter![],
				iter![],
				Some(&mut fence),
	...
	pub fn get_data(&self) -> Box<[u8]> {
		...
		let mut buffer = unsafe {
			self.context.device.create_buffer(
				(self.extent.width * self.extent.height) as u64 * 4,
		...
		let mut mem = unsafe {
			Box::from_raw(std::slice::from_raw_parts_mut(
				std::alloc::alloc(std::alloc::Layout::array::<u8>(size).expect("Array allocation size overflow")),
				(self.extent.width * self.extent.height) as usize * 4,
			) as *mut [u8])
		};
